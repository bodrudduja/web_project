<?php
	...
	if($rowSelected==1)
	{
		...
		if($rowSelected2==1)
		{
			...
			$sql3 = "SELECT official_name,designation,phone FROM hall_officials WHERE hall='".$hall_name."';";
			$result3 = mysqli_query($conn,$sql3);
			$rowSelected3 = mysqli_num_rows($result3);
			if($rowSelected3>0)
			{
				//$row3 = mysqli_fetch_array($result3);
				$data="<tr><th>Name</th><th>Designation</th><th>Phone</th></tr>";
				while($row3 = mysqli_fetch_array($result3))
				{	
					$data=$data."<tr>"."<td>" . $row3['official_name'] . "</td>"."<td>" . $row3['designation'] . "</td>"."<td>" . $row3['phone']."</td>"."</tr>";
}
